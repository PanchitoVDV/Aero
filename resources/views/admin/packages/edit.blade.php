@extends('layouts.app')
@section('title', 'Pakket Bewerken - ' . $package->name)
@section('header', 'Pakket Bewerken: ' . $package->name)

@section('content')
<div class="max-w-3xl">
    <form method="POST" action="{{ route('admin.packages.update', $package) }}" class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        @csrf
        @method('PUT')
        ...
        <div class="grid md:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input type="number" name="virtfusion_package_id" id="virtfusion_package_id" value="{{ old('virtfusion_package_id', $package->virtfusion_package_id) }}" required
                    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition text-sm">
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition text-sm">{{ old('description', $package->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            <select name="category" id="category" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition text-sm">
                <option value="vps" {{ $package->category === 'vps' ? 'selected' : '' }}>VPS</option>
                <option value="dedicated" {{ $package->category === 'dedicated' ? 'selected' : '' }}>Dedicated</option>
                <option value="game" {{ $package->category === 'game' ? 'selected' : '' }}>Game</option>
            </select>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
                ...
                <input type="number" name="cpu_cores" id="cpu_cores" value="{{ old('cpu_cores', $package->cpu_cores) }}" min="1" required
                    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition text-sm">
            </div>
            <div>
                <label for="memory" class="block text-sm font-medium text-gray-700 mb-1.5">RAM (MB)</label>
                <input type="number" name="memory" id="memory" value="{{ old('memory', $package->memory) }}" min="256" required
                    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition text-sm">
            </div>
            <div>
                <label for="storage" class="block text-sm font-medium text-gray-700 mb-1.5">Opslag (GB)</label>
                <input type="number" name="storage" id="storage" value="{{ old('storage', $package->storage) }}" min="5" required
                    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition text-sm">
            </div>
            <div>
                <label for="traffic" class="block text-sm font-medium text-gray-700 mb-1.5">Verkeer (GB)</label>
                <input type="number" name="traffic" id="traffic" value="{{ old('traffic', $package->traffic) }}" min="0" required
                    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition text-sm">
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
                ...
                <input type="number" step="0.01" name="price_monthly" id="price_monthly" value="{{ old('price_monthly', $package->price_monthly) }}" required
                    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition text-sm">
            </div>
            <div>
                <label for="price_quarterly" class="block text-sm font-medium text-gray-700 mb-1.5">Prijs/kwartaal (&euro;)</label>
                <input type="number" step="0.01" name="price_quarterly" id="price_quarterly" value="{{ old('price_quarterly', $package->price_quarterly) }}"
                    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition text-sm">
            </div>
            <div>
                <label for="price_yearly" class="block text-sm font-medium text-gray-700 mb-1.5">Prijs/jaar (&euro;)</label>
                <input type="number" step="0.01" name="price_yearly" id="price_yearly" value="{{ old('price_yearly', $package->price_yearly) }}"
                    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition text-sm">
            </div>
            <div>
                <label for="setup_fee" class="block text-sm font-medium text-gray-700 mb-1.5">Setup fee (&euro;)</label>
                <input type="number" step="0.01" name="setup_fee" id="setup_fee" value="{{ old('setup_fee', $package->setup_fee) }}"
                    class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition text-sm">
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            <button type="submit" class="bg-brand-600 text-white font-semibold px-6 py-2.5 rounded-lg hover:bg-brand-700 transition text-sm">Opslaan</button>
        </div>
    </form>
</div>
@endsection
